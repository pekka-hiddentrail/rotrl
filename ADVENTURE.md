...
│
├── 07_world_setting/              — World cosmology, geography, culture
...
│
├── 05_campaign_setting/           — Campaign-specific configuration
...
│
├── 06_books/                      — Published adventure content (human-reference; not injected at boot)
...
│
├── 08_persistence/                — Cross-session state tracking
...
│   └── white_deer/                 base.md
│
├── 02_events/                     — Event files (read by EventIndex at runtime)
...
3. RotRL adventure text  (canonical adventure path)
   ↓
4. World Operating Rules  (07_world_setting/WORLD_OPERATING_RULES.md)
   ↓
5. Golarion Canon  (07_world_setting/ lore)
   ↓
6. Campaign Setting  (05_campaign_setting/ customisations)
   ↓
7. Book / Act / Encounter specifics  (06_books/ — specific overrides general)
   ↓
...
3. Event map from `02_events/` via `EventIndex.event_map_text()` — appended if the directory is non-empty

**The `00_system_authority/`, `07_world_setting/`, `05_campaign_setting/`, and `06_books/` files are NOT currently loaded at boot.** The CORE BEHAVIOR and GM STYLE blocks are hardcoded strings in `_build_slim_system_prompt()`. The adventure_path files outside `01_npcs/`, `04_rules/skills/`, `03_locations/`, and `02_events/` are human-reference documents — valuable for authoring and review, but not injected into the LLM. Loading them at boot is a planned improvement (see TODO.md, `00_system_authority` loading section).
...
- **NPC profiles** — `01_npcs/{slug}/base.md` + `knowledge.md` + latest `session_NNN.md`
- **Skill rules** — `04_rules/skills/{skill}.md` (content above the `<!-- REFERENCE -->` marker only)
- **NPC-at-location** — NPCs whose `**Locations:**` field matches a location keyword
- **Location profiles** — `03_locations/{slug}/base.md` (content above `<!-- REFERENCE -->` only)
- **Active event content** — `session.active_events` entries are decremented each turn (TTL=5). Live events inject their content as `## Active Event — {id}` blocks. Events are fired when the LLM writes `%%EVENT%% <id>` and `02_events/<id>.md` exists.
...
Active events are TTL-bound — they expire automatically after 5 turns. The LLM is given an EVENT MAP in the system prompt (built from `02_events/` at boot) listing valid event IDs and their trigger conditions.
...
Session-generated locations (from `%%GENERATE%%` blocks with `type: location`) are created in `03_locations/{slug}/` immediately and the `LocationIndex` is invalidated so they are detectable on the very next turn. Unlike session NPCs, location stubs are not dot-prefixed and are not purged between sessions.
