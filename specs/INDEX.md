...
| [location-zones.feature](location-zones.feature) | Backend \| Frontend \| Content | `@location` `@zone` `@movement` `@ui` `@api` `@generator` `@context` | 13 | Location `## Zones` and `## Access Points`, `LocationIndex` zone graph parser, location zone state, `/location_zones`, `/zone_move`, Location Zones panel, generated draft zones |
