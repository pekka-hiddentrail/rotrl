...
3. ✔ The GM response does NOT show an attack banner in the Dice Tray — no player action required.
4. ✔ An `attack_result` entry appears in the DiceTray history with the ⚔ prefix.
...
3. ✔ The DiceTray shows an **attack banner** (not the normal skill-roll banner) with:
   - Attacker and target names
   - Attack bonus (e.g. `+5`)
   - Target AC
4. ✔ The `dice-tray-active` highlight (amber border) is on the panel.
...
4. ✔ DiceTray returns to its normal (non-active) state.
...
1. ✔ Attack log entries appear **above** the skill-roll history in the DiceTray.
...
5. ✔ The DiceTray attack banner still appears (the SSE event fires even though the token
...
4. ✔ DiceTray attack banner disappears — no ghost banner state.
5. ✔ DiceTray returns to the right column (combat-active layout reverts).
