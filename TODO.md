...
- [x] **Swallowtail Festival NPCs — full base.md files written** — 17 NPCs from the festival document now have complete `base.md` files (new format with GM Notes, Social Checks, Secrets, State Handling): Father Abstalar Zantus, Mayor Kendra Deverin, Sheriff Belor Hemlock, Cyrdak Drokkus, Ameiko Kaijitsu, Aldern Foxglove, Larz Rovanky, Alma Avertin, Brodert Quink, Niska Mvashti, Ven Vinder, Jargie Quinn, Turch Sterglus, Daviren Hosk, Rynshinn Povalli, Gaven Deverin, Naffer Vosk.
- [ ] **Tier A — remaining encounter anchors** (not yet in the festival doc): Savah Bevaniky (armory — equipment, Act I context), Risa Magravi (Hagfish tavern — rumours, Sczarni colour), Shayliss Vinder (General Store — Ven Vinder subplot). Write `base.md` for each: name, role, one-sentence personality, location anchor, what they know that's plot-relevant.
- [ ] **Tier B — district colour** (players will encounter if they explore): Hannah Velerin (healer, White Deer district), Das Korvut (blacksmith, grief subplot), Banny Harker (lumber mill, Act II body discovery), Ibor Thorn (lumber mill partner). Same format as Tier A.
...
Section-level context assembly — inject exactly what the scene needs, no more. `api/context/npc_extractor.py` is the extraction layer (done); these items build the strategy layer on top.
...
- [ ] **Second-pass prompt token reduction** — turn 1 currently lands at ~1609 tokens (target ~1235). Three levers: (1) boot.md "Who Is Present" NPC blurbs (~80 tokens) — redundant once NPC context injection fires on first mention; trim to name + location only. (2) EVENT MAP descriptions (~120 tokens) — shorten per-event trigger phrases to one clause each. (3) `_FORMAT_EXAMPLE` (~400 tokens) — drop one of the two `%%DELTAS%%` blocks in the example; single block is sufficient to teach the format. Do after session 1 is stable in production.

- [ ] **Smart context pruning** — when `len(system_content) > _GROQ_MAX_SYSTEM_CHARS`, the prompt is currently truncated at character 30,000 which can split mid-block. In M4, context becomes discrete blocks — pruning should drop whole blocks tail-first instead of truncating a string. *(blocked on M4)*
...
- [ ] **DC sourcing in the GM directive** — when both an NPC and a skill are detected on the same turn, `_build_turn_directive` should explicitly instruct the model to derive the DC from the NPC's stats and disposition (injected profile) and use the skill file only as a baseline range. Currently the model has both in context but no instruction to combine them, so it falls back to generic skill-file DCs regardless of who the player is dealing with.

- [ ] **Per-character skill ability overrides** — some PC traits replace the governing ability for a specific skill. `_inject_context` skill detection currently injects the raw skill file which assumes PF1e defaults (e.g. Knowledge (planes) → INT). Known overrides: **Planar Savant (Yanyeeku, Vanx)** — Knowledge (planes) uses CHA (+6) not INT (+1). Without this, any system-generated roll suggestion or DiceTray auto-bonus for Knowledge (planes) will be off by 5 for these two characters. Add `ability_overrides: dict[str, str]` to `pc_profiles` at boot time, populated from each character's traits list; `_inject_context` checks this map when building the skill briefing for a detected skill.
